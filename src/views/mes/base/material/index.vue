<template>
  <div>
    ...
    <!-- 新增/编辑弹窗 -->
    <a-modal v-model:open="open" :title="title" width="720px" @ok="submitForm" @cancel="open = false" :confirmLoading="submitting">
...
const form = reactive({ materialCode: '', materialName: '', materialType: 'RAW', categoryId: null, spec: '', model: '', unit: '', status: '0' })
...
const columns = [
  ...
  { title: '物料类型', dataIndex: 'materialType', align: 'center', width: 100 },
  { title: '规格', dataIndex: 'spec', align: 'center', width: 100 },
  { title: '型号', dataIndex: 'model', align: 'center', width: 100 },
...
function handleTreeSelect(keys) {
  ...
  proTableRef.value?.search({ categoryId: currentCategoryId.value })
}

function handleAdd() {
  Object.assign(form, { materialId: null, materialCode: '', materialName: '', materialType: 'RAW', categoryId: currentCategoryId.value || null, spec: '', model: '', unit: '', status: '0' })
...
function handleDelete(ids) {
  delMaterial(ids.join(',')).then(() => { message.success('删除成功'); proTableRef.value?.reload() })
}

function handleExport() {
  exportMaterial(proTableRef.value?.getSearchParams?.() || {}).then(() => message.success('导出成功'))
}
...
function submitForm() {
  formRef.value?.validate().then(() => {
    ...
    api({ ...form }).then(() => {
      message.success('保存成功'); open.value = false; submitting.value = false; proTableRef.value?.reload()
