...
const editLines = ref([])
const editLineColumns = [{ title: '物料编码', key: 'materialCode' }, { title: '物料名称', key: 'materialName' }, { title: '单位', key: 'unit' }, { title: '采购数量', key: 'orderQuantity' }, { title: '含税单价', key: 'unitPrice' }, { title: '操作', key: 'action', width: 60 }]
...
function openAdd() { editing.value = false; Object.keys(form).forEach(k => delete form[k]); fields.value.forEach(f => { form[f.key] = '' }); editLines.value = []; formOpen.value = true }
...
function addLine() { editLines.value.push({ _key: `new-${Date.now()}`, lineNo: editLines.value.length + 1, materialId: 0, materialCode: '', materialName: '', unit: '', orderQuantity: 0, unitPrice: 0 }) }
async function save() { const payload = { ...form }; if (type.value === 'order') payload.lines = editLines.value.map(({ _key, ...line }, index) => ({ ...line, lineNo: index + 1 })); await (editing.value ? config.value.api.update(payload) : config.value.api.add(payload)); message.success('保存成功'); formOpen.value = false; load() }
