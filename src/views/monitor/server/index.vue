          ...
          <a-card :loading="loading" title="内存" :bordered="false">
            <a-table
              ...
              <span slot="mem" slot-scope="text, record">
                <div :style="{
                  color: record.name == '使用率' && text > 80 ? '#ff4d4f' : '#262626',
                  textAlign: 'right',
                  fontWeight: record.name == '使用率' ? '600' : '400'
                }">
                  <a-icon type="warning" style="color:#faad14; margin-right: 4px;" v-if="record.name == '使用率' && text > 80" />
                  <span style="font-size: 14px;">{{ text }}</span>
                  <code style="margin-left: 4px; padding: 2px 4px; background: #f5f5f5; border-radius: 2px; font-size: 12px;" v-if="record.name == '使用率'">%</code>
                  <code style="margin-left: 4px; padding: 2px 4px; background: #f5f5f5; border-radius: 2px; font-size: 12px;" v-if="record.name != '使用率'">G</code>
                </div>
              </span>
              <span slot="jvm" slot-scope="text, record">
                <div :style="{
                  ...
                  <span style="font-size: 14px;">{{ text }}</span>
                  <code style="margin-left: 4px; padding: 2px 4px; background: #f5f5f5; border-radius: 2px; font-size: 12px;" v-if="record.name == '使用率'">%</code>
                  <code style="margin-left: 4px; padding: 2px 4px; background: #f5f5f5; border-radius: 2px; font-size: 12px;" v-if="record.name != '使用率'">M</code>
                </div>
              </span>
            </a-table>
          </a-card>
          ...
          <a-card :loading="loading" title="服务器信息" :bordered="false">
            <a-descriptions :column="2" v-if="server.sys">
              ...
              <a-descriptions-item label="操作系统">
                <template #default>{{ server.sys.osName }}</template>
              </a-descriptions-item>
...
// 访问配置（响应式）
const tableConfig = tableStore.config;
...
const getList = () => {
  getServer().then(response => {
    ...
    sysData.value = serverData.sysFiles
    setTimeout(() => {
      loading.value = false
    }, 500)
  })
